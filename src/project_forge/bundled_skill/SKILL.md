...
1. Run `project-forge doctor` and report missing required tools. Docker may be deferred only when
   the user accepts that Compose validation will remain pending.
...
5. Run `python harness/check.py` in the generated repository. Run Docker checks when Docker is
   available and the selected profile emits containers.
6. For an existing generated repository, require a clean Git worktree before `add`, `enable`, or
   `update`. Never bypass the guard. Report every `.rej` file and leave resolution to the user.
